...
const SignupForm = () => {
  ...
  const fieldError = (field) => (
    <p className={`mt-1 text-pink-600 text-sm ${formErrors[field] ? 'block' : 'hidden'}`} data-testid={`${field}-error`}>
    ...
    <form className="m-auto my-16 max-w-[500px] p-8 bg-white border-2 shadow-md rounded-md" onSubmit={handleSubmit}>
      {loading ? (
        ...
        <>
          ...
          <button type="submit" aria-label="Submit" className="bg-primary text-white px-4 py-2 font-medium hover:bg-primary-dark">
            Submit
          </button>
